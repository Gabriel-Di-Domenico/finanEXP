...
describe('Login', () => {
  it('Login with successfully', () => {
    ...
    const alias = TestUtils.batchRequestStub([AuthenticateRequests.authUser(true), AuthenticateRequests.verifyToken()]);
  ...
  it('Login fail', () => {
    ...
    cy.url().should('not.be.equal', 'http://localhost:4200/home');
  });
});
